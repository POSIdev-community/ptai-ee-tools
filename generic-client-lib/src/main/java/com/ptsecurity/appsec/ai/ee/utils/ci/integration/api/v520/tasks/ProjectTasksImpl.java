...
import com.ptsecurity.appsec.ai.ee.utils.ci.integration.api.v520.converters.AiProjConverter;
...
import static com.ptsecurity.appsec.ai.ee.server.v520.api.model.LegacyProgrammingLanguageGroup.*;
        ...
        ProjectModel projectModel = call(
                () -> {
                    try {
                        ...
                        log.trace("PT AI v.5.2.0 API returns HTTP status 204 if there's no project with given name {}", name);
        ...
        String result = call(
                () -> {
                    try {
                        ...
                        log.trace("PT AI v.5.2.0 API returns HTTP status 400 if there's no project with given Id {}", id);
