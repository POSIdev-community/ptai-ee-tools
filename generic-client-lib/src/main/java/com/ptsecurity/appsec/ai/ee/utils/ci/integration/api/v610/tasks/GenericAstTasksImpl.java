...
import com.ptsecurity.appsec.ai.ee.utils.ci.integration.api.v610.ApiClient;
...
import com.ptsecurity.appsec.ai.ee.utils.ci.integration.api.v610.converters.IssuesConverter;
import com.ptsecurity.appsec.ai.ee.utils.ci.integration.api.v610.converters.ScanErrorsConverter;
...
import static com.ptsecurity.appsec.ai.ee.utils.ci.integration.api.v610.converters.ApiErrorCode.SCAN_ALREADY_SCHEDULED;
import static com.ptsecurity.appsec.ai.ee.utils.ci.integration.api.v610.converters.IssuesConverter.convert;
import static com.ptsecurity.misc.tools.helpers.CallHelper.call;

@Slf4j
public class GenericAstTasksImpl extends AbstractTaskImpl implements GenericAstTasks, BranchTask {
    private final ScanResultIdHelper scanResultIdHelper = new ScanResultIdHelper(
            client.getScanQueueApi(),
            client.getBranchesApi(),
            client.getProjectsApi());
    ...
    public UUID startScan(
        ...
        Scope scope = fullScanMode ? Scope.FULL : Scope.INCREMENTAL;
        createQueueItem.setScope(scope);
        if (scanLabel != null && !scanLabel.trim().isEmpty()) {
            createQueueItem.setScanLabel(scanLabel);
        }
    ...
    private UUID getQueueItemId(UUID projectId, String branchName, CreateQueueItem createQueueItem) {
        ...
        try {
            queueItemId = call(
                    () -> client.getScanQueueApi().createItem(createQueueItem),
        ...
        public void run() {
            ...
            while (true) {
                try {
                    ...
                    ScanProgressModel scanProgress = call(
                            () -> client.getProjectsApi()
                                    .apiProjectsProjectIdScanResultsScanResultIdProgressGet(projectId, scanResultId),
                            "Get project scan progress failed");

                    reset();
                    // TODO: Properly process this
                    if (null == scanProgress) break;
                    if (null == scanProgress.getStage()) break;
                    @NonNull Stage stage = EnumsConverter.convert(scanProgress.getStage());
    ...
    public void stop(@NonNull UUID scanResultId) throws GenericException {
        ...
        List<QueueItem> queueItems = call(
                () -> client.getScanQueueApi().getAllItems(),
                "Failed to get queue items");

        List<UUID> queueItemsToDelete = queueItems.stream()
                .filter(i -> scanResultId.equals(i.getScanResultId()))
                .map(QueueItem::getId)
                .collect(Collectors.toList());

        DeleteQueueItems deleteQueueItems = new DeleteQueueItems();
        deleteQueueItems.setItems(queueItemsToDelete);

        UUID queueItemId = queueItemsToDelete.stream()
                .findFirst()
                .orElse(null);
