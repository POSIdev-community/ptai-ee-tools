...
import com.ptsecurity.appsec.ai.ee.server.v500.api.model.ScanResultModel;
import com.ptsecurity.appsec.ai.ee.server.v500.api.model.ScanSettingsModel;
import com.ptsecurity.appsec.ai.ee.server.v500.api.model.VulnerabilityModel;
...
public class ConverterTest extends BaseTest {
    @SneakyThrows
    public ScanResult generateScanResult500(@NonNull final String fileName) {
        ObjectMapper mapper = createObjectMapper();
        log.trace("Read scan results");
        String scanResultStr = getResourceString("v500/json/scanResult/" + fileName + ".json");
        ScanResultModel scanResult = mapper.readValue(scanResultStr, ScanResultModel.class);
        log.trace("Read scan issues");
        String scanIssuesStr = getResourceString("v500/json/issuesModel/" + fileName + ".json");
        ...
        for (Reports.Locale locale : Reports.Locale.values()) {
            Path issuesFile = extractResourceFile("v500/json/issuesModel/" + fileName + "." + locale.getLocale().getLanguage() + ".json.7z");
        ...
        @NonNull final ScanSettingsModel scanSettings = reader.with(DeserializationFeature.READ_ENUMS_USING_TO_STRING).readValue(
                getResourceString("v500/json/scanSettings/" + fileName + ".json"),
                ScanSettingsModel.class
        );
        Map<ServerVersionTasks.Component, String> versions = new HashMap<>();
        versions.put(ServerVersionTasks.Component.AIE, "5.0.0.39484");
        versions.put(ServerVersionTasks.Component.AIC, "5.0.0.39484");

        String projectName = StringUtils.substringBefore(fileName, ".");

        return com.ptsecurity.appsec.ai.ee.utils.ci.integration.api.v500.converters.IssuesConverter.convert(projectName, scanResult, issues, issuesHeadersFiles, scanSettings, "https://ptai500.domain.org", versions);
    }

    @Test
    @DisplayName("Convert PT AI 5.0.0 scan results")
    @SneakyThrows
    public void generateScanResults() {
        try (TempFile destination = TempFile.createFolder()) {
            Path scanResults500 = destination.toPath().resolve("result").resolve("v500");
            assertTrue(scanResults500.toFile().mkdirs());

            for (ProjectTemplate.ID templateId : ProjectTemplate.ID.values()) {
                ProjectTemplate projectTemplate = getTemplate(templateId);
                ScanResult scanResult = generateScanResult500(projectTemplate.getName());
                String json = BaseJsonHelper.minimize(scanResult);
                packData7Zip(scanResults500.resolve(projectTemplate.getName() + ".json.7z"), json.getBytes(StandardCharsets.UTF_8));
            }
