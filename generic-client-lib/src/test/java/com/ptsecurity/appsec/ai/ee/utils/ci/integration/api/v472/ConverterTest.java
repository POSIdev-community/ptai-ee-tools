...
import com.ptsecurity.appsec.ai.ee.server.v472.api.model.ScanResultModel;
import com.ptsecurity.appsec.ai.ee.server.v472.api.model.ScanSettingsModel;
import com.ptsecurity.appsec.ai.ee.server.v472.api.model.VulnerabilityModel;
...
public class ConverterTest extends BaseTest {
    @SneakyThrows
    public ScanResult generateScanResult472(@NonNull final String fileName) {
        ObjectMapper mapper = createObjectMapper();
        log.trace("Read scan results");
        String scanResultStr = getResourceString("v472/json/scanResult/" + fileName + ".json");
        ScanResultModel scanResult = mapper.readValue(scanResultStr, ScanResultModel.class);
        log.trace("Read scan issues");
        String scanIssuesStr = getResourceString("v472/json/issuesModel/" + fileName + ".json");
        ...
        for (Reports.Locale locale : Reports.Locale.values()) {
            Path issuesFile = extractResourceFile("v472/json/issuesModel/" + fileName + "." + locale.getLocale().getLanguage() + ".json.7z");
        ...
        @NonNull final ScanSettingsModel scanSettings = reader.with(DeserializationFeature.READ_ENUMS_USING_TO_STRING).readValue(
                getResourceString("v472/json/scanSettings/" + fileName + ".json"),
        ...
        return com.ptsecurity.appsec.ai.ee.utils.ci.integration.api.v472.converters.IssuesConverter.convert(projectName, scanResult, issues, issuesHeadersFiles, scanSettings, "https://ptai472.domain.org", versions);
    }
    ...
    public void generateScanResults() {
        try (TempFile destination = TempFile.createFolder()) {
            Path scanResults472 = destination.toPath().resolve("result").resolve("v472");
            assertTrue(scanResults472.toFile().mkdirs());

            for (ProjectTemplate.ID templateId : ProjectTemplate.ID.values()) {
                ProjectTemplate projectTemplate = getTemplate(templateId);
                ScanResult scanResult = generateScanResult472(projectTemplate.getName());
                String json = BaseJsonHelper.minimize(scanResult);
                packData7Zip(scanResults472.resolve(projectTemplate.getName() + ".json.7z"), json.getBytes(StandardCharsets.UTF_8));
            }
