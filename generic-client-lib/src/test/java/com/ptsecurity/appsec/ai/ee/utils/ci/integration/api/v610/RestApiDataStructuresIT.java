...
import com.ptsecurity.appsec.ai.ee.server.v610.api.ApiResponse;
...
@Slf4j
@DisplayName("Test PT AI 6.1.0 REST API data structures")
@Tag("development")
...
public class RestApiDataStructuresIT extends BaseClientIT {
    ...
    protected void generateData(@NonNull final Path destination, @NonNull final ProjectTemplate.ID templateId, @NonNull final Consumer<UnifiedAiProjScanSettings> modifySettings) {
        ...
        com.ptsecurity.appsec.ai.ee.utils.ci.integration.api.v610.ApiClient client = new ApiClient(connectionSettings);
        // Initialize all API clients with URL, timeouts, SSL settings etc.
        client.init();
        client.authenticate();

        Path jsons = destination.resolve("v610").resolve("json");
        jsons.toFile().mkdirs();
