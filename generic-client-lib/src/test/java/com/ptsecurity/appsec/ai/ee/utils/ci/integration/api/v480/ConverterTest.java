...
import com.ptsecurity.appsec.ai.ee.server.v480.api.model.ScanResultModel;
import com.ptsecurity.appsec.ai.ee.server.v480.api.model.ScanSettingsModel;
import com.ptsecurity.appsec.ai.ee.server.v480.api.model.VulnerabilityModel;
...
public class ConverterTest extends BaseTest {
    @SneakyThrows
    public ScanResult generateScanResult480(@NonNull final String fileName) {
        ObjectMapper mapper = createObjectMapper();
        log.trace("Read scan results");
        String scanResultStr = getResourceString("v480/json/scanResult/" + fileName + ".json");
        ScanResultModel scanResult = mapper.readValue(scanResultStr, ScanResultModel.class);
        log.trace("Read scan issues");
        String scanIssuesStr = getResourceString("v480/json/issuesModel/" + fileName + ".json");
        ...
        for (Reports.Locale locale : Reports.Locale.values()) {
            Path issuesFile = extractResourceFile("v480/json/issuesModel/" + fileName + "." + locale.getLocale().getLanguage() + ".json.7z");
        ...
        @NonNull final ScanSettingsModel scanSettings = reader.with(DeserializationFeature.READ_ENUMS_USING_TO_STRING).readValue(
                getResourceString("v480/json/scanSettings/" + fileName + ".json"),
                ScanSettingsModel.class
        );
        Map<ServerVersionTasks.Component, String> versions = new HashMap<>();
        versions.put(ServerVersionTasks.Component.AIE, "4.8.0.39484");
        versions.put(ServerVersionTasks.Component.AIC, "4.8.0.39484");

        String projectName = StringUtils.substringBefore(fileName, ".");

        return com.ptsecurity.appsec.ai.ee.utils.ci.integration.api.v480.converters.IssuesConverter.convert(projectName, scanResult, issues, issuesHeadersFiles, scanSettings, "https://ptai480.domain.org", versions);
    }

    @Test
    @DisplayName("Convert PT AI 4.8.0 scan results")
    @SneakyThrows
    public void generateScanResults() {
        try (TempFile destination = TempFile.createFolder()) {
            Path scanResults480 = destination.toPath().resolve("result").resolve("v480");
            assertTrue(scanResults480.toFile().mkdirs());

            for (ProjectTemplate.ID templateId : ProjectTemplate.ID.values()) {
                ProjectTemplate projectTemplate = getTemplate(templateId);
                ScanResult scanResult = generateScanResult480(projectTemplate.getName());
                String json = BaseJsonHelper.minimize(scanResult);
                packData7Zip(scanResults480.resolve(projectTemplate.getName() + ".json.7z"), json.getBytes(StandardCharsets.UTF_8));
            }
